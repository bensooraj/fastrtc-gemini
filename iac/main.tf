...
# HTTPS listener that uses an existing ACM certificate
resource "aws_lb_listener" "https" {
  load_balancer_arn = aws_lb.app.arn
  port              = 443
  protocol          = "HTTPS"
  ssl_policy        = "ELBSecurityPolicy-2016-08" # or a stricter policy you prefer
...
resource "aws_instance" "app" {
  ...
  root_block_device {
    volume_size = 30 # Set to 30 GiB or more as needed
    volume_type = "gp3"
  }
